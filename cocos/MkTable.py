...
class Table:
    ...
    def __init__(self, from_input, max_attrs, modifier_input: ReadAttributesConcept = None, two_modifiers=False):
        self.two_modifiers = two_modifiers

        if two_modifiers:  # from_input contiene la KB con un head e 2 modificatori
            self.title = from_input.title
            self.h_conc = from_input.head_conc
            self.m_conc = from_input.mod_conc
            self.m2_conc = from_input.mod2_conc
            self.attrs = from_input.attrs
            self.typical_attrs = from_input.typical_attrs

        elif modifier_input is None:  # 1 head, 1 modifier in un solo file
            self.title = from_input.title
            self.h_conc = from_input.head_conc
            self.m_conc = from_input.mod_conc
            self.attrs = from_input.attrs
        ...
        else:  # 1 head e 1 modifier in due file di input
            head_input = from_input
            self.title = head_input.title + "_" + modifier_input.title
            self.h_conc = head_input.title
            self.m_conc = modifier_input.title
            self.attrs = head_input.attrs
            self.attrs.extend(modifier_input.attrs)
            self.typical_attrs = head_input.typical_attrs
            self.typical_attrs.extend(modifier_input.typical_attrs)
    ...
    def createTable(self, max_attrs):
        w, h = len(self.typical_attrs), pow(2, len(self.typical_attrs))
    ...
    #   questo metodo scandisce ognuna di queste linee e 
        ...
        for line in range(len(self.table)):
            ...
            for x in range(len(self.table[line])):
                if (self.table[line][x] == '1'):
                    tot *= self.typical_attrs[x][1]
                else:
                    tot *= 1 - self.typical_attrs[x][1]
    ...
    # Metodo che stampa la tabella (ordinata)
    def printTable(self):
        i = len(self.sorted_table) - 1
        while i >= 0:
            print(str(self.sorted_table[i]) + '')
            i -= 1
    ...
    def delNotConsistent(self):
        ...
        while i < len(self.table):
            ont = ManageOntology(typical_attrs=self.typical_attrs, attrs=self.attrs, line=self.table[i],
                                 two_modifiers=self.two_modifiers)
            consistent = ont.is_consistent()
            # print(str(consistent)+', '+str(self.table[i][:-1])+'\n')
            print('Scenario numero ' + str(i) + ': ' + str(self.table[i]) + '\n\n\n')
            if not consistent:
                del self.table[i]
                i -= 1
            i += 1
...
if __name__ == "__main__":
    ...
    from_input = ReadAttributes('Pet_Fish_3_concepts', two_modifiers=True)
    tab = Table(from_input, two_modifiers=True)
    pprint(tab.table)
    pprint(tab.typical_attrs)
    pprint(tab.attrs)
