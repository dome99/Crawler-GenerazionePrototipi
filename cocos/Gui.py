...
#qui sotto
    def scaleMoved(self, event) :
        ...
        for x in range(len(app.table.typical_attrs)):
            if (app.table.sorted_table[int(self.scale.get_value()) - 1][x] == '1'):
                used_attrs += "    "
                used_attrs += (app.table.typical_attrs[x][0])

                if (app.table.typical_attrs[x][2]):
                    used_attrs += (" (from " + app.table.h_conc + ")\n")
                else:
                    used_attrs += (" (from " + app.table.m_conc + ")\n")

        self.label.set_text("The scale is on percentage " +
                            str(app.table.sorted_table[int(self.scale.get_value()) - 1][len(app.table.typical_attrs)]) +
                            "%\n" + used_attrs)
